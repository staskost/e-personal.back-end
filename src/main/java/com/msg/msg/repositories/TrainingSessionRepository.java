...
public interface TrainingSessionRepository extends JpaRepository<TrainingSession, Integer> {
	...
	List<TrainingSession> findByTrainerAndCancelationStatusAndReadCancelationStatus(User trainer, int status,
			int status2);

	List<TrainingSession> findByClientAndCancelationStatusAndReadCancelationStatus(User client, int status,
			int status2);
